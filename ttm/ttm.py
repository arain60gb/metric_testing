...
#         if 'File_Path' in self.random_sample and 'array' in self.random_sample['File_Path'] and 'sampling_rate' in self.random_sample['File_Path']:
#             audio_array = self.random_sample['File_Path']['array']
#             sample_rate = self.random_sample['File_Path']['sampling_rate']
...
#             return audio_path
#         else:
#             print("No audio found in the sample.")
#             return None
...
class MusicGenerationService(AIModelService):
    ...
    def handle_music_output(self, axon, music_output, prompt, model_name):
        try:
            # Convert the list to a tensor
            music_tensor = torch.Tensor(music_output)
            ...
            if check_duplicate_music(audio_hash):
                ...
                bt.logging.info(f"Aggregated Score from Smoothness, SNR and Consistency Metric: {score}")
                self.update_score(axon, score, service="Text-To-Music")
                return output_path
